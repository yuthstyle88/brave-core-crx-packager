...
const S3_EXTENSIONS_BUCKET = process.env.S3_EXTENSIONS_BUCKET || 'ibrowe-core-ext'
...
const isUpdateCRXFile = (endpoint, region,  id, hash) => {
  getItemFromId(endpoint, region,  id).then((item) => {
    return !item || item.hash.S !== hash
  })
}
...
const addCommonScriptOptions = (command) => {
  return command
    ...
    .option('-e, --endpoint <endpoint>', 'DynamoDB endpoint to connect to', 'http://localhost:4566')
